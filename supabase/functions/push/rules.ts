...
export function whatToSay(
  w: Waiting, lastSent: string | null, now: Date, shop = ""
  ...
  // Which shop, under the news. Somebody who helps at two of them needs to
  // know before they put their boots on, and the name is painted over the
  // door — it is not a figure out of the till, which is what has no business
  // on a lock screen.
  const under = [...lines.slice(1), shop.trim()].filter(Boolean);
