-- Money off one line, rather than off the whole sale.
--
-- "Ten percent off the ladder" is a different transaction from "ten percent off
-- the lot", and until now only the second could be recorded. A cashier wanting
-- the first had to work out what it came to and take it off the sale total,
-- which lands on the invoice as an unexplained lump and spreads itself back
-- across every line pro-rata — so the ladder shows full price and the padlocks
-- quietly carry a discount nobody gave them.
--
-- THE ARITHMETIC, because an invoice that does not add up is worse than one
-- without discounts at all:
--
--   sales.subtotal        Σ (qty × unit_price), before anything comes off
--   sales.discount_amount Σ (line discounts) + the sale-level discount
--   sales.total           subtotal − discount_amount
--
-- so subtotal − discount = total still reads correctly on the slip, whichever
-- kind of discount was given. A line keeps its OWN discount in
-- sale_items.discount_amount; line_total stays what it has always been —
-- after this line's own discount and after its share of the sale-level one.
--
-- The percentage is stored beside the amount when that is how it was given. The
-- amount is what counts and what the invoice shows; the percentage is kept so
-- the slip can say "10% off" and a manager reading it back next week can see
-- what was agreed rather than reverse-engineering it from two figures.
...
declare
  ...
  -- Per-line discounts, and what they come to across the sale.
...
begin
  ...
  -- Any discount needs approving, not only a sale-level one. Ten percent off a
  -- ladder is the same money as ten percent off a sale with a ladder in it.
  v_status := 'completed';
...
-- The reprint and the closer look both need to see a line's own discount.
-- Dropped and recreated rather than replaced: the return columns change.
